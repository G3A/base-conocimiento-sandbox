...
# This repository has no `make audit` target and no org.owasp:dependency-check-maven already
# configured in pom.xml, so the fallback applies: versions:display-dependency-updates, always
# available via the Maven wrapper.
#
# -DprocessDependencyManagement=false scopes the report to the ~20 dependencies this pom actually
# declares. Without it, the plugin also walks every transitive dependency pulled in through the
# four imported BOMs (spring-modulith-bom, spring-ai-bom, testcontainers-bom, arconia-bom) and
# prints thousands of lines — measured at 3400+ on this repo — which would flood the agent's
# context at the start of every session instead of reporting.
OUT="$(./mvnw -f pom.xml versions:display-dependency-updates -DprocessDependencyManagement=false 2>&1)"
STATUS=$?
