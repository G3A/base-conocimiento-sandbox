...
Localmente, `make hooks` instala Lefthook: el pre-commit corre `spotless:check` si hay `.java`
staged y escanea secretos en lo staged; el pre-push corre `make check`.
...
`/sdlc-ia:instrument-project-java` (etapa F2 de `validacion-workshop/`, en el repositorio del workshop) y
...
mensajería (Kafka/RabbitMQ/JMS) detectados. Señal de IA: `spring-ai-*` (ver arriba) y
la instrumentación para agentes ya está en el repo: `.mcp.json` y los hooks del agente en
`scripts/agent-hooks/`, instalados por `/sdlc-ia:instrument-agent-java` en la etapa F2 de la
validación. Ver [infrastructure.md](infrastructure.md#agente-de-ia-hooks-y-mcp).
